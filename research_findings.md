...
- **Augmentation to kill shortcuts (the challenge's exact warning):**
  - Strong color jitter + hue + RandomGrayscale(p≈0.1 to 0.2): contrastive/texture-bias work shows nets cheat via color/texture; destroying it forces structure. *(Geirhos texture-bias ICLR 2019; SimCLR.)* [HIGH, OOD-proven]
  - **MixStyle** (mix feature mean/std across samples) at EARLY layers only (last-block collapses it): PACS LOO +3.3 to 4.2. *(Zhou et al., ICLR 2021.)* [HIGH], apply cautiously to ConvNeXt (LayerNorm); A/B it.
...
- **Resolution:** iterate at 224, finalize at 384 warm-started from 224 (~+1 to 1.5pt); fine particulate/thin bands are small-scale → higher res helps. Given tall aspect, consider a tall input (e.g., 384×224 or 448×256) to preserve vertical detail with less distortion.
- **GeM pooling** unconditionally (cheap upgrade over GAP, p≈2 to 3). Optional texture branch (iSQRT-COV / bilinear) for turbidity-scattering; multi-scale/row-band features for vertical layering. [MED]
- **Schedule:** AdamW + cosine/one-cycle, layer-wise LR decay ~0.65 to 0.75, short warmup, EMA, AMP, batch 16 to 32. [HIGH]
